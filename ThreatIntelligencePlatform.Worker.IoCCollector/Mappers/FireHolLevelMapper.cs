...
public class FireHolLevelMapper : Profile
{
    public FireHolLevelMapper()
    {
        CreateMap<FireHolLevelResponseDto, IoCDto>()
            .ForMember(dest => dest.Id, opt => opt.MapFrom(src => (string?)null))
            .ForMember(dest => dest.Source, opt => opt.MapFrom(src => SourceName.FireHolLevel.ToString()))
